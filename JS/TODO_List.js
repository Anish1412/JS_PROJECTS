...
function retainTask(id) {
  ...
  document.querySelectorAll("li")[id].innerHTML = li+"<input type='checkbox' style='margin-left:10px;"+
  "height:18px; width:18px' onclick='cancelTask("+id+")'>"+
  "<button type='button' class='btn2' onclick='updateTask("+id+")'>Update Task</button>"+
  "<button type='button' class='btn1' onclick='removeTask("+id+")'>Delete Task</button>";
  document.querySelectorAll("li")[id].style.opacity = "1";
  console.log(li);
}

function cancelTask(id) {
  let li = document.querySelectorAll("#demo")[0].children[id].firstChild.textContent;
  document.querySelectorAll("li")[id].innerHTML =
    "<s>"+li+"<input type='checkbox' style='margin-left:10px; height:18px; width:18px' onclick='retainTask("+id+")'></s>";
  document.querySelectorAll("li")[id].style.opacity = "0.3";
  document.querySelectorAll("input")[id].toggleAttribute("checked");
}
...
function updateTask(id) {
  ...
  if(newContent){
    ...
    document.querySelectorAll("#demo")[0].children[id].firstChild.textContent = newContent;
  }
}
